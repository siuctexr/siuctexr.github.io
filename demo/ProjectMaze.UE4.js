...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_ProjectMaze', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202206171455, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../ProjectMaze/ProjectMaze.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var wasmLocation = "https://saluki-my.sharepoint.com/:u:/g/personal/bijayraj_paudel_siu_edu/EaXWsvtyB7FLuTF8g51jlgEBzMceDetn6nTJ0YyO3JZhpg?e=pqQOsa"
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile(wasmLocation), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('ProjectMaze.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Utility.js')).then(addScriptToDom);
		var fileLocation = 'https://saluki-my.sharepoint.com/:u:/g/personal/bijayraj_paudel_siu_edu/Ef504Rdkd_1Ksf_qxerdOJoBVhhrlWxHU_hUXq1Bs2rpfw?e=sQxKvx'
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('ProjectMaze.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('ProjectMaze.data')] = dataArrayBuffer;
...
// fetchOrDownloadAndStore(db, Module.locateFile('ProjectMaze.data'), 'arraybuffer').then(function(dataArrayBuffer) {
// 				Module['preloadedPackages'] = {};
// 				Module['preloadedPackages'][Module.locateFile('ProjectMaze.data')] = dataArrayBuffer;
// 				return dataJsDownload.then(addScriptToDom);
// 			});

		fetchOrDownloadAndStore(db, Module.locateFile(fileLocation), 'arraybuffer').then(function(dataArrayBuffer) {
			Module['preloadedPackages'] = {};
			Module['preloadedPackages'][Module.locateFile(fileLocation)] = dataArrayBuffer;
			return dataJsDownload.then(addScriptToDom);
		});
